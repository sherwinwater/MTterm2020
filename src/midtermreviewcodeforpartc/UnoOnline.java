...
/**
 * The signIn page for UnoOnline. Takes in a user name and password and then validates the password
 * before creating a new user in the list of possible users. To be used as the beginning code for
 * MidtermReview C.
 *
 * @author dancye, 2019
 */
public class UnoOnline {
    ...
    /**
     * method that takes in the User's name and chosen password and then continues prompting until
     * the password is valid based on the following two rules: 1. The password must be at least
     * length 8 2. The password must contain at least one "special character"
     *
     */
    private void run() {
        int userCount =0;
        ...
        PasswordValidator one = new PasswordValidator ();
        String password = one.validateAll();
